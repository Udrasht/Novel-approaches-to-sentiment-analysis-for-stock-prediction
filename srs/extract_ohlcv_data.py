...
def combine_ohlc_news( ohlc_df, news_df ):
    #ohlc_df, news_df = ohlc_dict[ticker], stock_news_dict[ticker].reset_index()
    df11 = ohlc_df.copy()
    df12 = news_df.copy()
...
#stock_news_dict = news.fetch_news( all_tickers, start_dt_str = beg_date, till_dt_str = end_date )
folder_name = 'data'
#add_financials = 0
#add_indicators = 1
#add_news = 1
...
#combined_data_dict[ticker] = combine_ohlc_news( ohlc_dict[ticker], stock_news_dict[ticker].reset_index()  )
...
#    filename = ticker + "_combine.csv" 
#    data_path = folder_name + "\\" + filename
#    combined_data_dict[ticker] = pd.read_csv(data_path).set_index('datetime')