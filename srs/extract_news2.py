...
#stories = get_titles( stock, '2023-01-11', '2023-01-13')
...
def fetch_news( all_tickers, start_dt_str, till_dt_str, time_delta_orig = 5):
    ...
    #["INTU", "PYPL", "ADBE", "NVDA", "ORCL", "EBAY", "AMZN", "NFLX", "GM"]
...
def generate_full_summary(day, lst):
    ...
    cnt1 += 1
    #print( 'Day',cnt1,':', cnt2,'/',len(lst) )
    print( cnt1,').' ,'Day', day,':', cnt2,'/',len(lst) )
    #return new_lst
    #return [ get_summary(url) for url in lst ]
...
#summ = get_summary( url = stock_news_dict['AAPL']['link'][0][0] )


#stock_news_dict['AAPL']['summary'] = stock_news_dict['AAPL']['link'].apply(generate_full_summary)
#stock_news_dict['MSFT']['summary'] = stock_news_dict['MSFT']['link'].apply(generate_full_summary)


#len(stock_news_dict['AAPL']['summary'][1][1])
...
#beg_date = '2023-01-01'
end_date = '2022-12-31'
no_of_threads = 50          #only for summary, not for title 
